...
p <-
  ...
  filter(IID>name) %>%
  mutate(pop=case_when(Country.x=="Argentina"&Country.y=="Argentina" ~ "Argentina",
                       Country.x=="Chile"&Country.y=="Chile" ~ "Chile",
                       TRUE ~ "between")
  ) %>%
  mutate(pop=factor(pop, levels = c("Argentina", "Chile", "between"))) %>%
  arrange(pop) %>%
  ggplot() +
  geom_jitter(aes(x=geo, y=gen, shape=pop, color=pop)) +
  scale_shape_manual(name="pairs",
